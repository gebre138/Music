...
const MusicList: React.FC<Props> = ({ tracks, onEdit }) => {
  ...
  if (!tracks?.length)
    return <p className="text-gray-500 text-sm">No tracks available</p>;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {tracks.map((track) => (
          <div
            key={track.sound_id}
            className="bg-white rounded-lg shadow-md border flex flex-col overflow-hidden hover:shadow-lg transition"
          >
            {/* Album Image */}
            <div className="w-full h-32">
              <img
                src={track.album_file_url || "/placeholder.png"}
            ...
            <div className="p-3 flex flex-col flex-1">
              ...
              <div className="mt-2 flex gap-2 flex-wrap text-xs">
                <span className="bg-gray-200 text-pink-600 px-2 py-0.5 rounded-full">
                  {track.category}
                </span>
              ...
              <div className="mt-3 flex justify-between items-center">
                ...
                <button
                  ...
                  onClick={() =>
                    setExpanded(expanded === track.sound_id ? null : track.sound_id)
                  }
                >
              ...
              {expanded === track.sound_id && (
                <div className="mt-2 text-xs text-gray-700 grid grid-cols-1 md:grid-cols-2 gap-2">
                  {FORM_FIELDS.filter((f) => f.type !== "file").map((f) => (
                    <p key={f.name}>
                      <strong>{f.label}:</strong> {track[f.name as keyof Track] || "-"}
                    </p>
                  ))}
                </div>
              )}
